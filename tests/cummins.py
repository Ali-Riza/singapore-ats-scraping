...
# =========================
# CONFIG (CUMMINS / JOBSYN SOLR)
# =========================

COMPANY = "Cummins"
SOURCE = "jobsyn_solr"

CAREERS_URL = "https://cummins.jobs/"
...
# Headers based on your curl (these matter for this API)
HEADERS = {
    "accept": "application/json, text/plain, */*",
    "accept-language": "en-US,en;q=0.9",
    "origin": "https://cummins.jobs",
...
def _normalize_date(v: Any) -> Optional[str]:
    ...
    if len(s) >= 10 and s[4] == "-" and s[7] == "-":
        # try datetime first, else keep YYYY-MM-DD
        try:
...
def _format_location(job: dict) -> str:
    # Try a few common field patterns
    loc = _pick(
        job,
        [
            "location",
            ...
            "jobLocation",
            "job_location",
            # common Solr-ish variants
            "location_s",
            "location_display",
            "location_text",
            "job_location_s",
            # Jobsyn/NLX variants seen in Cummins response
            "location_exact",
        ],
    )
    ...
    parts = [p for p in [_clean_text(city), _clean_text(state), _clean_text(country)] if p]
...
def _extract_job_url(job: dict) -> str:
    ...
    if isinstance(url, str) and url.strip():
        u = url.strip()
        # Sometimes API returns relative URLs
        if u.startswith("/"):
            return f"https://cummins.jobs{u}"
        return u
        ...
        for page in range(1, max_pages + 1):
            ...
            if debug and page == 1:
                if isinstance(payload, dict):
                    ...
                    resp = payload.get("response")
                    if isinstance(resp, dict):
                        print("response_keys(sample):", sorted(list(resp.keys()))[:40])
                ...
                if jobs_raw:
                    ...
                    if isinstance(first, dict):
                        ...
                        for k in (
                            ...
                            if k in first:
                                print(f"first_doc[{k}] =", first.get(k))
            ...
            for job in jobs_raw:
                ...
                job_id = _extract_job_id(job)
                # Dedupe: prefer job_id; fall back to url+title if no id
            ...
            # Stop if a page yields no new unique jobs
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Cummins (jobsyn Solr) scraper for Singapore.")
