...
"""MSA Safety (careers.msasafety.com) scraper.

Same approach as tests/schneider_electric.py:
- The listings UI is a Jibe search app.
- Use the JSON endpoint at: https://careers.msasafety.com/api/jobs

Goal: print one dict per job:
{
...
START_URL = (
	"https://careers.msasafety.com/jobs?stretchUnit=MILES&stretch=10"
	"&location=Singapore,%20Singapore&woe=7&regionCode=SG"
)
...
def _job_url_from_api(job: dict) -> Optional[str]:
	...
	if slug:
		return f"https://careers.msasafety.com/jobs/{slug}?lang={job.get('language','en-us')}"
	return None
	...
	*,
	...
	with requests.Session() as session:
		...
		for pnum in range(1, max_pages + 1):
			...
			r = session.get("https://careers.msasafety.com/api/jobs", params=params, timeout=30)
			r.raise_for_status()
			payload = r.json()
...
def main() -> None:
	ap = argparse.ArgumentParser(description="Scrape MSA Safety jobs and print Yinson-style dicts.")
